...
ui <- fluidPage(
  titlePanel("Zenodo Metadata Builder"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      checkboxGroupInput("selected_authors", "Select Authors", choices = NULL),
      textInput("keywords", "Keywords (separated by semicolons)"),
      dateInput("pub_date", "Publication Date", value = Sys.Date()),
      selectInput("access_right", "Access Right", choices = c("open", "embargoed", "restricted", "closed")),
      textInput("license", "License (e.g. CC-BY-4.0)", value = "CC-BY-4.0"),
      actionButton("add", "Add Entry"),
      downloadButton("download_json", "Download JSON"),
      hr(),
      h4("Add New Author"),
      textInput("author_name", "Full Name (e.g. Smith, John)"),
      textInput("author_affiliation", "Affiliation"),
      textInput("author_orcid", "ORCID (optional, full URL)"),
